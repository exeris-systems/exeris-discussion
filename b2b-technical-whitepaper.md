...
| **Exeris Spring Runtime** | `exeris-spring-runtime` | Host-runtime integration — Spring as application framework on Exeris-owned ingress, lifecycle, and ScopedValue context (ADR-010, ADR-011). The brownfield adoption path. |
...
**Service Boundary family** (IDP, PIM, OMS, Headless CMS) layers in `exeris-spring-runtime` as the **Tier 1 host runtime** for the external API surface; the cap composition itself (`service-boundary-core` + `rest-emission` / `graphql-emission` + platform-layer caps from §3.2 layer 4 — multi-tenancy, audit, RBAC, attachments, search, workflow — + domain primitives from §3.2 layer 5) is Spring-free. Spring `@RestController` paths in SKU code consume the caps' `@Provides` services; the caps themselves never import Spring. The host-runtime selection is orthogonal to the driver swap of §3.3 above: a Service Boundary SKU runs the same cap manifest whether the substrate driver is Community or Enterprise, and could in principle run under a non-Spring host runtime in the future without manifest changes. Heavy lifting (saga state, off-heap document processing, NUMA-aware compute paths at Enterprise tier) is kernel-level. Brownfield-migrating customers can adopt one Service Boundary SKU incrementally, leaving the rest of their Spring estate untouched.
...
Independent SaaS products in the Exeris Systems family (BudgetHQ being the first) run on the same kernel and Spring Runtime as platform subscribers. They are commercially independent: their pricing, customer base, branding, and roadmap are determined by their own product strategy, not by the platform's. Family products consume platform capabilities through the same `@Provides` / `@Requires` mechanism platform subscribers do, with no special access. This is detailed in §8.
