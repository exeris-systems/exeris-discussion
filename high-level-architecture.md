...
| Host-Spring integration | Pure Mode + Compatibility Mode | ADR-010, ADR-011 | Adopt incrementally |
...
> **Note on host-runtime independence.** No cap `@Requires` `exeris-spring-runtime`. The Wall (§4 below) extends to capabilities: caps cannot reach into Spring internals. Spring Runtime is a **Tier 1 host-runtime selection** — a substrate choice orthogonal to driver selection (Community/Enterprise) — that an SKU manifest layers in at Tier 3 to host Spring `@RestController` paths over Exeris-owned ingress. Service Boundary–family SKUs select Spring Runtime as their host because they target brownfield Spring-on-Exeris migration; Gateway-family SKUs select no host runtime (data plane is kernel-direct). The cap manifest is byte-identical in both cases — the host-runtime choice does not bleed into `@Requires` of any cap. Spring `@RestController` paths (user / SKU code, not cap code) consume cap `@Provides` services; the dependency arrow never reverses.
...
**Two orthogonal Tier 1 substrate selections.** A deployment selects (a) a kernel driver — `exeris-kernel-community` Maven module (Community: NIO H1/H2 + portable Off-Heap TLS over TCP) or `exeris-kernel-enterprise` (Enterprise: `io_uring`/IOCP transport, `EnterpriseQuicTlsEngine`, HTTP/3, slab pools, NUMA) — and (b) a host runtime — kernel-direct (no host) or `exeris-spring-runtime` for Spring `@RestController` paths over Exeris-owned ingress. **Both selections are Tier 1 substrate decisions that never enter `@Requires` of any cap.** A cap manifest is identical across {Community, Enterprise} × {kernel-direct, Spring-hosted}; the substrate selections layer in at SKU manifest time.
...
| IDP | Service Boundary | Spring Runtime Pure Mode (API surface) + kernel-level (AI Abstraction caps) | NUMA-aware allocation under heavy doc-processing load | Cloud or on-prem; air-gappable |
| PIM | Service Boundary | Spring Runtime Pure Mode | Marginal — Community driver typically sufficient | Cloud or on-prem |
| OMS | Service Boundary | Spring Runtime Pure Mode + kernel-level (L4 Flow saga state) | Marginal — Community driver typically sufficient | Cloud; distributed saga state requires Postgres |
| Headless CMS | Service Boundary | Spring Runtime Pure Mode | Marginal — Community driver typically sufficient | Cloud or edge-co-located for read replicas |
...
**Service Boundary family architecture.** SB-family SKU manifests select **Spring Runtime as the Tier 1 host runtime** for the external API surface — this is an SKU-level substrate selection, not a cap dependency. The host runtime supplies the Spring `@RestController` execution context on Exeris-owned ingress with ScopedValue context propagation; user / SKU controller code consumes `service-boundary-core`'s `ApiSurfaceRegistry` and `RequestContext` `@Provides` services. The caps themselves never import Spring types — `service-boundary-core` and every platform cap in §3.2 layer 4 `@Requires` only kernel SPIs. Heavy lifting (off-heap document processing for IDP, graph-attribute traversal for PIM, saga state for OMS, content-as-domain emission for Headless CMS) happens at the kernel level via composed caps. Service Boundary SKUs demonstrate that brownfield migration from existing Spring-based vertical SaaS is structurally possible — a customer can adopt one Service Boundary SKU incrementally without rewriting the rest of their estate. Symmetrically, a Service Boundary SKU could in principle ship a non-Spring host runtime in the future without touching the cap manifest, because the host runtime is a Tier 1 selection.
...
**Spring Runtime relationship per SKU family.** Spring Runtime is a **Tier 1 host-runtime selection** that an SKU manifest layers in at Tier 3; it is never a cap `@Requires`. Gateway SKUs do **not** select Spring Runtime — they run as a kernel-direct data plane, consistent with the clarified ADR-021 placing gateway-class workloads outside the `exeris-spring-runtime` scope. Service Boundary SKUs select Spring Runtime in Pure Mode as their host runtime for the external API surface, providing the structural validation that brownfield migration works. Family products (BudgetHQ, §9) select Spring Runtime in Pure Mode for their entire application stack — this is the dogfooding case. In every case the cap composition manifest is identical with or without Spring Runtime layered in; caps do not depend on Spring.
...
## 7. Spring Adoption Paths

### 7.1 Pure Mode (default, recommended)

Exeris-owned ingress, no servlet/reactive stack, ScopedValue context propagation. Performance-first; `ThreadLocal` banned on hot paths. ADR-010 + ADR-011.

### 7.2 Compatibility Mode (opt-in)

Opt-in via `exeris.runtime.web.mode=compatibility`. Narrow ThreadLocal bridging where strictly necessary (e.g. `SecurityContextHolder`); isolated in `*.compat.*` sub-packages and never auto-active in Pure Mode. JDBC compatibility (ADR-017) is opt-in within Compatibility Mode and documents specific bypass semantics (`PersistenceAdmissionStageEvent` bypass, per-statement JFR events, `PersistenceErrorTranslator`).
...
### 7.4 BudgetHQ as the structural validation

BudgetHQ (the first Family product, §9) is built entirely on Spring Runtime Pure Mode, consuming external APIs (Tink, Salt Edge, Stripe) through the platform's capability-level adapters. The bank-aggregator capability is prototyped inside BudgetHQ first and later promoted to the platform's reusable capability ecosystem; the receipt-scan capability bridges to the IDP capability via the platform's AI Abstraction Layer SPI; the subscription-billing capability is prototyped inside BudgetHQ as a Stripe adapter and is on the promotion path to a reusable platform cap.

This is the structural validation of the platform's brownfield claim: a real product runs on the platform from day one, on the same SPI a paying subscriber would consume, with no special privileges. The dogfooding feedback loop generates production-load bug reports and performance data that no synthetic benchmark can match.
...
- Runs on Exeris Spring Runtime, typically in Pure Mode.
